...
**重要な限界（2026-09-23 第4回更新）**：本セッションの実行環境では WebFetch（一次資料の本文取得）が egress ポリシーによりブロックされており、当初は WebSearch の抄録レベル要約に依存していたが、**利用者からのPDF提供により、13件中12件（L1, L2, L3, L5, L6, L7, L8, L9, L10, L11, L12, L13）を全文確認済み**とした。L6はIOPscience購読ページからは本文を取得できなかったが、NIHMS/PMC著者最終稿という別ルートで全文を取得できた。**唯一未確認の L4（Yan et al. 2026, Phys Med Biol）は、購読制のため2回試行しても取得できず、利用者の判断により抄録レベルの情報で確定させることとした。** 全文確認により、当初の暫定評価は複数箇所で更新・精密化されている（特に L1 の発見により貢献A/拡張①の新規性主張を再度絞り込み、L7 により貢献Bの`c`パラメータに対応する実測アンカー値を獲得した——§3.1, §3.2参照）。本表は12/13件が全文確認済みであり、確定度は非常に高い。
...
| L7 | Delker A, Schleske M, Liubchenko G, Berg I, Zacherl MJ, Brendel M, Gildehaus FJ, Rumiantcev M, Resch S, Hürkamp K, Wenter V, Unterrainer LM, Bartenstein P, Ziegler SI, Beyer L, Böning G. "Biodistribution and dosimetry for combined [¹⁷⁷Lu]Lu-PSMA-I&T/[²²⁵Ac]Ac-PSMA-I&T therapy using multi-isotope quantitative SPECT imaging." *Eur J Nucl Med Mol Imaging*. 2023;50(5):1280–1290. doi:10.1007/s00259-022-06092-1（**全文確認済み 2026-09-23、LMU München**） | 2023 | EJNMMI | 患者（臓器/voxel、腎臓・病変26segmented lesions） | ¹⁷⁷Lu＋²²⁵Ac（**8患者へ同時投与**、真の同時Dual-isotope SPECT） | Siemens Symbia Intevo T16 SPECT/CTで440 keV(²¹³Bi)と208 keV(¹⁷⁷Lu)を同一1時間スキャンで同時取得（24h後）、48h後は¹⁷⁷Luのみ追加。MAP-EM再構成、Gaussianポストフィルタ(FWHM 30/40mm)でノイズ調整 | **腎臓SUV相関：Pearson r=0.94 (p<0.01)、病変SUV相関：r=0.96 (p<0.01)**。回帰式：腎臓 y=1.05x−0.44、病変 y=1.32x−0.31。腎臓SUVは²²⁵Acが²¹⁷⁷Luより平均21±23%高い（遊離²¹³Biの腎集積が一因と考察）。RBE加重線量（RBE=5）：腎臓は²²⁵Acが¹⁷⁷Luより平均32±35%高い（p<0.01、最大106%）。病変線量は有意差なし（p=0.54） | **極めて重要な発見——本研究の`c`（空間共局在度）パラメータに対応する実測値を提供する一次資料。** 実際に同時投与された患者の²²⁵Ac/¹⁷⁷Lu SUV相関（腎臓r=0.94、病変r=0.96）は、本計画書§5.3で定義した実測対応量`ρ`（Pearson相関）そのものであり、**実臨床でのPSMA同時投与療法における共局在度が「ほぼ完全共局在だが完全には一致しない」（r≈0.94-0.96、回帰直線の傾きも1からずれる）ことを直接示す実測アンカーデータ**となる。本研究のPhase 3a（`f×c`位相図）で探索する`c`のうち、臨床的に最も妥当な範囲を`c≈0.9〜1.0`域に設定する根拠として、またPaper IIIで理論モデルを実測値と対比させる際の直接的な参照値として活用できる。依然として各核種の空間分布は実測SPECT画像をそのまま使用しており、`c`を明示的パラメータとして操作する解析ではなく、また物理線量`D_Ac+D_Lu`を明示的に合算した混合場としての解析も行っていない——**貢献Bの新規性そのものは影響を受けない** |
...
| L12 | Peter R, Bidkar AP, Bobba KN, Zerefa L, Dasari C, Meher N, Wadhwa A, Oskowitz A, Liu B, Miller BW, Vetter K, Flavell RR, Seo Y. "3D small-scale dosimetry and tumor control of ²²⁵Ac radiopharmaceuticals for prostate cancer." *Sci Rep*. 2024;14:19938. doi:10.1038/s41598-024-70417-3（**全文確認済み 2026-09-23、UC Berkeley/UCSF**） | 2024 | Scientific Reports | µm（iQIDオートラジオグラフィ 39×39×210µm voxel、200µm間隔スライス→3D再構成） | ²²⁵Ac連鎖（GATEで個別核種シミュレーション、分岐比考慮）＋遊離²¹³Bi補正 | 22Rv1異種移植マウスに[²²⁵Ac]Ac-Macropa-PEG₄-YS5（CD46標的）投与、24h/168h時点でiQIDデジタルオートラジオグラフィ＋H&E染色＋γ線分光による²¹³Bi再分布補正（macro-to-micro dosimetry）。TCP計算はLQモデルでα=1.8 Gy⁻¹、**β≈0（線形近似）**を採用 | 腫瘍からの²¹³Bi再分布は「有意に観測されず」（tumor Ab(t)/Aa(t)=1 within uncertainty）。**腎臓では対照的に遊離²¹³Biの寄与が支配的**：核種別線量（Table 2）で²¹³Bi 0.26-0.32 Gy/kBq vs ²²⁵Ac自体 0.08-0.09 Gy/kBq——**腎臓線量の70-80%が遊離²¹³Bi由来**。3D DAR法の空間精度はγ解析で87±6%（3ピクセル・10%許容） | ²²⁵Ac単独の微視的不均一性研究、Ac/Lu併用ではない（貢献Bには影響なし）。**H3（娘核種再分布の重要性）への強力な独立支持証拠**：腫瘍では再分布が無視できる一方、腎臓では遊離²¹³Biが総線量の大半を占めるという「標的臓器と非標的臓器で再分布の重要性が逆転する」パターンは、L9（Wurzer et al. 2025）の知見と独立に一致する。**さらにL10と同様、²²⁵AcのTCPモデルに線形（β≈0）応答を採用しており、§5.6のLQ交差項の前提再検討をさらに補強する第3の独立証拠**となる |
...
6. **【新規】L7（Delker et al. 2023）の全文確認により、`c`パラメータの実測アンカー値を獲得した。** 8患者への²²⁵Ac/¹⁷⁷Lu同時投与実データで、腎臓SUV相関 r=0.94、病変SUV相関 r=0.96 が報告されている。これは本計画書§5.3で定義した実測対応量`ρ`そのものであり、**PSMA同時投与療法における実際の共局在度は「ほぼ完全だが完全ではない」（r≈0.94–0.96）**ことを直接示す。本研究のPhase 3a探索範囲（c=0, 0.25, 0.5, 0.75, 1.0）に、この臨床的に妥当な高共局在域（c≈0.9–1.0）での細かい刻みを追加する根拠となる。

**総合評価**：貢献Bに関する差別化は、L5・L6・L7・L8・L9・L10という6件の直接関連文献の全文確認を経て、一貫して支持され続けている。特にL7は「脅威」ではなく、むしろ本研究のモデルパラメータに実測較正値を与える**協力的な先行研究**として機能する。
...
## 4. 全体としての結論（13件中12件を全文確認、2026-09-23 第4回・最終更新）
...
貢献B（コア）については、L1・L5・L6・L7・L8・L9・L10のいずれも²²⁵Acと¹⁷⁷Luを`f`・`c`を独立変数とした混合場として扱っておらず、当初の評価を覆す証拠は見つからなかった。

**最終ラウンドで追加確認したL7・L12は、いずれも本研究計画にとって有益な結果をもたらした。**

- **L7（Delker et al. 2023）**：実際に²²⁵Ac/¹⁷⁷Luを同時投与された8患者のSUV相関（腎臓r=0.94、病変r=0.96）を報告しており、本研究の`c`パラメータに対応する**実測アンカー値**を初めて得た。これはPhase 3aの探索範囲設計（臨床的に妥当な高共局在域を重点的にカバーする）、および将来的にPaper IIIの理論モデルを実データと対比させる際の直接的な参照値として活用できる。
- **L12（Peter et al. 2024）**：²²⁵Ac単独の解析だが、腎臓線量の70-80%が遊離²¹³Bi由来であることを定量的に示しており、L9（Wurzer et al. 2025）と独立に一致する強力な追加証拠となった。またTCPモデルでβ≈0（線形）を採用しており、L10と合わせて**2件の独立文献**が²²⁵Acの線量反応関係の線形性を支持している——§5.6のLQ交差項見直しの必要性がさらに強まった。

**唯一未確認のL4（抄録情報のみで確定）を除き、Phase 0暫定検索における一次資料確認はほぼ完了したとみなす。** 総じて、本研究計画のコア（貢献B）は12件の関連文献の精査を経てなお独自性を維持しており、拡張①（貢献A）は当初より狭いが依然として妥当な範囲に絞り込まれ、かつ複数の実測アンカー値（L9, L12, L13の²¹³Bi再分布データ、L7の共局在度データ）によってパラメータ設計の根拠が大幅に強化された。正式なPhase 0（大学データベース経由の系統的レビュー、PRISMA-ScR準拠）では、本表を出発点として検索網羅性を高め、特にP6（核データ評価文献）の特定と、貢献C・D（時間構造・4因子統合UQ）に関する先行研究の有無を追加確認する。

---

## 5. 文献確認ステータス（最終更新：2026-09-23 第4回）

**13件中12件が全文確認済み。実質的にPhase 0暫定検索の一次資料確認は完了とみなす。**

### 唯一の未達（利用者判断により抄録情報で確定）
- **L4**：Yan K, Jiang Y, Wang R, et al. A fast convolution-based method for microdosimetric comparison of ²²⁵Ac, ²¹¹At, ¹⁷⁷Lu and ¹⁶¹Tb at the cell cluster scale. *Phys Med Biol*. 2026;71:125005. doi:10.1088/1361-6560/ae7892 **※IOPscience購読制のため2回試行しても本文入手できず。利用者の指示により、抄録から得られる情報（本表L4の記載内容）を最終版として確定する。** 抄録レベルでも、本計画のPhase 2（畳み込みエンジン）との方法論的近さ、および単一核種ごとの独立比較（2核種混合ではない）という結論は明確であり、貢献A・Bへの評価に実質的な影響はないと判断する。
