...
**この4点は理論的推測ではなく、実際に文献で確認されたスケール依存性によっても裏付けられる**（`docs/phase0-literature-extraction.md`）。Liubchenko et al. 2024（臓器/voxelスケール、患者SPECT）は、²²¹Frと²¹³Biの薬物動態を厳密に分離しても腎臓線量の推定値は単一光子ピークを代用した場合と比べて最大8%しか変わらないと報告している。一方 Koniar et al. 2023（細胞スケール、GATE Monte Carlo）は、同じ²²¹Fr・²¹³Biの保持率を単一細胞の自己線量S値レベルで変化させると、最大72%もの差が生じることを示している。**同じ物理現象（娘核種再分布）が、評価スケールによって「ほぼ無視できる」から「支配的」まで結論が逆転する**——これは本研究がマクロな臨床線量評価だけでなく微視的スケールでの評価を必要とする理由を、既存の一次データで直接裏付けるものである。
...
> **文献検証について**：本節（§2）は、当初のWebSearchベースの暫定調査を経て、利用者提供のPDFにより13件中12件を全文精読して確定させたものである（唯一未達のL4は抄録情報で確定）。検証の詳細な経緯・各論文からの逐語的根拠は `docs/phase0-literature-extraction.md` に記録している。以下は、その全文検証を経た**確定版**の記述であり、検証過程の変遷は上記の別紙に譲る。

### 2.2 領域ごとの成熟度マップ（確定版、12/13件全文確認済み）

| 領域 | 成熟度 | 根拠 | 本研究の関与 | トラック |
|---|---|---|---|---|
| ¹⁷⁷Lu 単核種 organ/voxel dosimetry | ◎ 確立 | — | 参照・再現検証 | — |
| ²²⁵Ac 単核種 organ/voxel dosimetry | ◎ 確立 | — | 参照・再現検証 | — |
| ¹⁷⁷Lu DPK / 細胞S値 | ◎ 確立 | — | 再現検証（ベンチマーク） | コア |
| ²²⁵Ac DPK / 細胞S値（連鎖一括・実効カーネル） | ◎ 確立 | L1, L3 | 再現検証の対象そのもの | コア |
| ²²⁵Ac/¹⁷⁷Lu 併用の臨床 dosimetry（臓器/voxelスケール、実測`c`データを含む） | ◎ 実施済 | L7 | 参照・比較対象、`c`の実測アンカー（ρ=0.94–0.96） | — |
| µm分解能マルチ核種DPK/畳み込みフレームワーク（Phase 2相当の手法） | ◎ 確立 | L2, L3, L4 | 参照・整合性確保。畳み込み手法自体の新規性は主張しない | — |
| ²²⁵Ac 崩壊連鎖を核種別に分解した DPK 群 | ◎ 確立（離散的幾何配置として） | L1, L2 | 実効カーネルの適用限界の検証に限定 | 拡張① |
| 放射能比 `f` を連続変数とした統一混合カーネル（完全共局在前提） | △ 部分的 | L3, L4, L11 | 完全共局在の基準線として再現 | コア |
| **空間共局在度 `c` を明示変数とした混合線量場の一般式**（2核種同時投与） | **弱**（最も近い L5・L6・L7 も「単核種比較」または「実測相関値」に留まり、`f×c`独立変数化は未達） | L5, L6, L7 | **主要貢献（コア）** | コア |
| 娘核種再分布の臓器スケール定量化（daughter-specific PK） | ◎ 確立 | L8, L9 | 拡張①の`η_i`パラメータ範囲の実測根拠 | — |
| 娘核種の保持率を連続変数化し空間的に再分配するモデル | △（Koniarが20%刻みで近似的に実施済み、連続かつ空間再分配のモデルは未達） | L1 | 貢献（拡張、範囲を絞り込み） | 拡張① |
| 時間依存放射能比 `f(t)` と線量率効果の統合 | 弱（競合文献見つからず） | — | 貢献（拡張） | 拡張② |
| 生物学的効果の非加算性（LQ交差項）、α/β・RBE文献値 | 弱〜研究あり（²²⁵Acは線形近似が主流という新知見） | L10, L12 | 貢献（拡張、パラメータ取得元） | 拡張② |
| 混合場における分解能・カットオフ由来不確かさの定量化 | 非常に弱 | — | 貢献（コアの一部：数値検証として） | コア |
| 4 因子統合 UQ（分散寄与の順序付け） | 非常に弱（競合文献見つからず） | — | **統合目標（拡張）** | 拡張③ |
| ²²⁵Ac 崩壊連鎖の核データ評価 | ◎ 確立（別目的の基盤文献） | P6 | Phase 1 の核データソースとして採用 | — |
| 不均一分布下でのα/β核種「選択」問題 | 2026年時点でも未解決と明言する一次資料あり（学術的重みは限定的） | L11 | 状況証拠・問題の具体例として引用 | — |

### 2.3 研究ギャップの記述（確定版）

> ²²⁵Ac および ¹⁷⁷Lu については、それぞれ独立に検証された Monte Carlo ベースの微視的線量カーネル（DPK・細胞 S 値、離散的な核種分解を含む）が確立している。両核種を同時投与する治療に対する患者個別 dosimetry も実施されており、そこでは実測される SUV 相関（腎臓 ρ=0.94、病変 ρ=0.96；Delker et al. 2023）が、両核種の分布が「ほぼ完全だが完全には一致しない」ことを示している。複数核種を共通の微視的線量評価枠組みで扱う研究、単一核種の娘核種保持率を離散的にスイープする感度解析、および臓器スケールでの娘核種別薬物動態の分離も、いずれも既に報告されている。
>
> 一方、²²⁵Ac/¹⁷⁷Lu 併用療法において、**両核種を同時投与する混合場を放射能比 `f` と空間共局在度 `c` の独立変数として定式化し、`D(r; f, c)` の位相図を評価した研究は確認できなかった**。既存の最も近い先行研究（Tranel et al. 2021, 2022）は、2 核種のどちらか一方を選択する問題として空間分離を扱っており、両核種を同時に混合投与する設計ではない。また、娘核種の保持率を連続変数として空間的に再分配するモデル（本研究の `η_i` 定式化）も、既存研究（Koniar et al. 2023）の離散的な感度解析とは異なる新規性を残している。
>
> 時間依存放射能比 `f(t)` と線量率効果の統合、および 4 因子（空間共局在・娘核種再分布・放射能比・数値誤差）を統合した不確かさ定量化についても、直接の先行研究は確認できなかった。

**このうち本研究がコアとして確実に完遂するのは空間共局在度（`f` との同時取り扱いを含む）である**。娘核種再分布・時間構造・生物学的非加算性は拡張研究として、コアの成果を土台に時間の許す範囲で取り組む（§12 参照）。

**差別化の要点（貢献B・コア）**：最も近い3件の先行研究との違いは以下の通り、いずれも全文確認により確定している。
- Tranel et al. 2022（L5）・2021（L6）：常に単一核種のみを線源として評価しており、`f`（放射能比）という変数が存在しない。空間パラメータ（`Lmean`）も2細胞集団間の距離であり、本研究の`c`（2核種分布間の相関）とは異なる幾何定義である。
- Delker et al. 2023（L7）：実際の同時投与患者データを提供する点で最も価値が高いが、各核種の分布は実測SPECT画像をそのまま用いており、`c`を明示的パラメータとして操作した解析ではない。この`ρ=0.94–0.96`という実測値は、脅威ではなくむしろ本研究のモデルパラメータの実測較正値として活用する（§5.3, Phase 3a）。

**差別化の要点（貢献A・拡張①）**：Koniar et al. 2023（L1）が²²¹Fr/²¹³Bi保持率を0–100%・20%刻みでスイープする感度解析を単一細胞スケールで既に実施しているため、本研究の新規性は「η_iの導入」自体ではなく、(i) 遊離核種の線量を消失させず独立分布`p_i^free(r)`へ明示的に再分配すること、(ii) 臓器スケールの実測再分布データ（L8, L9, L12, L13）を`η_i`として微視的スケールに接続すること、(iii) `η_i`が混合場（Ac+Lu併用）の最適`f`・`c`設計に与える影響を評価すること、の3点に絞り込まれる。

**§5.6（生物学的効果層）への影響**：Rumiantcev et al. 2023（L10）・Peter et al. 2024（L12）という独立した2件の文献が、²²⁵Acの線量反応関係を線形（LQではない、`β_Ac≈0`）としてモデル化している。これは当初想定していたLQ交差項`2√(β_Ac β_Lu)D_Ac D_Lu`の前提を再検討する必要があることを示す重要な知見であり、拡張②の設計に反映済みである（§5.6参照）。
...
| A | **`η_i` の空間的再分配モデルへの拡張、および混合場文脈での感度評価** | ²²⁵Ac 崩壊連鎖を親核種一括ではなく 7 核種（²²⁵Ac, ²²¹Fr, ²¹⁷At, ²¹³Bi, ²¹³Po, ²⁰⁹Tl, ²⁰⁹Pb）に分解し、各核種の保持率 `η_i ∈ [0,1]` を連続変数として、束縛分布 `p_parent(r)` と**独立分布**`p_i^free(r)`の線形補間でモデル化する（線量保存則あり） | **核種分解カーネル自体、および保持率を0–100%でスイープする感度解析自体は、Koniar et al. 2023（単一細胞スケール、20%刻み）・Hu et al. 2025（細胞核、6離散幾何配置）で既に実施済みと判明した（`docs/phase0-literature-extraction.md` L1, L2）。** したがって本貢献はさらに絞り込んで、(i) 遊離核種の線量を「消失」ではなく独立分布`p_i^free(r)`へ**明示的に空間再分配**すること（先行研究は遊離分の線量を系から除くのみで行き先をモデル化しない）、(ii) 臓器スケールで既に実測されている再分布データ（Unterrainer/Liubchenko et al. 2024, Wurzer et al. 2025 等）を`η_i`として微視的スケールに接続すること、(iii) `η_i`が**混合場（²²⁵Ac+¹⁷⁷Lu併用）の最適`f`・`c`設計に与える影響**を評価すること（先行研究はいずれも²²⁵Ac単核種の解析）に新規性を絞り込む | 拡張① |
...
| パラメータ | 範囲 |
|---|---|
| ²¹³Bi 保持率 `η_Bi` | 0.3, 0.5, 0.7, 0.9, 1.0（**実測アンカー（全文確認済み）**：de Kruijff et al. 2019 はポリマーソーム担体（100/200 nm、静注／腫瘍内投与）で腫瘍内²¹³Bi保持比を条件別に報告しており、0.58–0.91の範囲に収まる（1日目/7日目×100nm/200nm粒子の4条件）。本研究の探索範囲はこのレンジをほぼ過不足なくカバーする。`docs/phase0-literature-extraction.md` L13。なお同論文は親核種²²⁵Ac自体の担体内保持も約93%（100%ではない）と報告しており、ナノ粒子担体特有の追加誤差の存在も示唆する） |
...
| R1' | Phase 0 で貢献 A（拡張）が既に満たされている先行研究が発見される | 拡張①の新規性喪失（コアには影響しない） | 拡張①を貢献 C（時間構造、拡張②）または D（UQ、拡張③）に差し替える。コアは貢献 B のみに立脚しているため学位論文の根幹は揺るがない<br>**暫定検索結果（Hu et al. 2025、Ghaseminejad et al. 2025を全文確認）**：**R1'は部分的に顕在化した**。核種分解カーネル自体（Koniar et al. 2023; Hu et al. 2025）は既に存在するため、貢献Aの主張を「8核種モデルの提案」から「保持率`η_i`の連続変数化」に狭めた（§4）。**全文確認により、Hu et al. 2025の「6分布」が離散的幾何カテゴリ（核/細胞質/全細胞/細胞外/内外両方/膜結合）であり連続的な保持率パラメータではないことを確定した**ため、この絞り込み後の貢献Aは新規性を保っていると判断する。**さらにGhaseminejad et al. 2025（L3）を全文確認したところ、²²⁵Ac連鎖を単一の実効カーネル（Whole Chain DPK）として扱っており核種分解自体を行っていないことが判明し、Phase 1コア（実効カーネル）／拡張①（核種分解）という本計画の段階分けが実際の文献の手法分布と自然に対応していることが確認できた**。**さらにKoniar et al. 2023（L1）の全文確認により、保持率η_iの0–100%スイープ自体が単一細胞スケールで既に実施済みであることが判明し、拡張①の新規性を「η_iの空間的再分配モデルへの拡張、および混合場文脈での感度評価」にさらに一段絞り込んだ（§4）**。当初計画の想定内の事象（R1想定の縮小版）であり、対応済み——**ただし拡張①の残された新規性の幅は当初よりもかなり狭く、Phase 6での位置づけ（拡張、コアではない）が一層妥当であることが確認された** |
...
- [P6]（**特定完了、全文未確認**）Huang X, et al. Evaluation of ²²⁵Ac decay data. *Appl Radiat Isot*. 2007;65(6):712–723. PubMed 17383887 — ENSDF準拠の²²⁵Ac崩壊連鎖の完全な崩壊データ評価。より新しい候補文献（"Half-life and gamma-ray emission intensity measurements for ²²⁵Ac and progeny", *Appl Radiat Isot*, 2025年頃）も存在し、Phase 1核データ選定時にどちらを一次ソースとするか確認する。ICRP Publication 107 と併用
